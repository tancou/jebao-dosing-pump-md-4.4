...
const debug = require('debug')('jebao:discovery');

const JEBAO_UDP_PORT = 12414;

/**
 * This class allows to discover Jebao devices in the same network via UDP messages
 */
class JebaoDiscovery extends EventEmitter {
    ...
    discover() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.socket.bind(undefined, this.options.listenAddress || '0.0.0.0', () => {
                ...
                debug('Listen for local JEABO devices on port 12414');
                ...
                try {
                    this.socket.send(probeData, 0, probeData.length, JEBAO_UDP_PORT, '255.255.255.255');
                } catch (e) {
                    this.socket.emit('error', e);
                }
